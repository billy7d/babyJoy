...
async function inspectSummary(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      promotionSize: numericFontSize(".cart-summary .promotion-breakdown-value strong"),
      ...
      promotionValue: box(".cart-summary .promotion-breakdown-value"),
...
try {
  for (const viewport of [
    ...
    try {
      ...
      await page.locator(".promotion-breakdown").waitFor({ state: "visible", timeout: 10000 });
      ...
      assert(metrics.rowDisplay === "missing", `Cart summary ${viewport.width}px còn row Khuyến mãi tổng hợp`);
      assert(metrics.breakdownHeadingDisplay !== "none", `Thiếu label Ưu đãi đang áp dụng ở ${viewport.width}px`);
      assert(metrics.breakdownAmountDisplay !== "none", `Thiếu amount promotion ở ${viewport.width}px`);
      assert(metrics.promotionName && metrics.promotionValue, `Thiếu phần tử applied promotion ở ${viewport.width}px`);
      assert(
        metrics.promotionName.right <= metrics.promotionValue.left + 1,
        `Tên promotion chưa nằm ở cột trái của value ở ${viewport.width}px`,
      );
      assert(
        metrics.promotionBreakdown &&
        metrics.breakdownBeforeTotal,
        `Applied promotion chưa nằm giữa subtotal và total ở ${viewport.width}px`,
      );
      ...
      assert(
        metrics.text.includes("Miễn phí vận chuyển") &&
          metrics.text.includes("- Free Shipping"),
        `Thiếu nội dung free shipping ở ${viewport.width}px`,
      );
